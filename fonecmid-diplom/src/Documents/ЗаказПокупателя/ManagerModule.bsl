...
Процедура ВКМ_МассовоСоздатьАкты(ПараметрыКоманды, ПараметрыВыполненияКоманды) Экспорт

	СсылкиЗаказов = ВКМ_ПодготовитьСсылкиЗаказов(ПараметрыКоманды);
	...
	Если ТипЗнч(ПараметрыВыполненияКоманды) = Тип("Структура") Тогда
		ПараметрыВыполненияКоманды.Свойство("Уникальность", ИдентификаторОперации);
	КонецЕсли;
...
// ВКМ_Изменение_Начало
Функция ВКМ_ПодготовитьСсылкиЗаказов(ПараметрыКоманды)

	СсылкиЗаказов = Новый Массив;

	Если ТипЗнч(ПараметрыКоманды) = Тип("Массив") Тогда
		Для Каждого СсылкаЗаказа Из ПараметрыКоманды Цикл
			Если ТипЗнч(СсылкаЗаказа) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
				СсылкиЗаказов.Добавить(СсылкаЗаказа);
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ТипЗнч(ПараметрыКоманды) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		СсылкиЗаказов.Добавить(ПараметрыКоманды);
	КонецЕсли;
...
Функция ВКМ_СоздатьАктыПоЗаказамВФоне(СсылкиЗаказов) Экспорт

	Результат = Новый Структура("Создано,Пропущено,Ошибки", 0, 0, 0);
	Всего = СсылкиЗаказов.Количество();
	Если Всего = 0 Тогда
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого СсылкаЗаказа Из СсылкиЗаказов Цикл
		...
		НомерОбработки = НомерОбработки + 1;

		// ВКМ_Изменение_Начало
		Попытка
			НачатьТранзакцию();
			ВКМ_ЗаблокироватьОснованиеРеализации(СсылкаЗаказа);

			Если ВКМ_ЕстьАктПоОснованию(СсылкаЗаказа) Тогда
			...
			Реализация.Заполнить(СсылкаЗаказа);
		...
		ПроцентВыполнения = Цел(НомерОбработки * 100 / Всего);
...
Процедура ВКМ_ЗаблокироватьОснованиеРеализации(СсылкаЗаказа)

	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("Документ.РеализацияТоваровУслуг");
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	ЭлементБлокировки.УстановитьЗначение("Основание", СсылкаЗаказа);
	Блокировка.Заблокировать();

КонецПроцедуры

Функция ВКМ_ЕстьАктПоОснованию(СсылкаЗаказа)
	...
	Запрос.УстановитьПараметр("Основание", СсылкаЗаказа);
